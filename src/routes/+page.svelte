<script>
    ...
    async function onSubmit(event) {
        if (!file) {
            ...
            making = 1;
            console.log(image64)
            components.update(n => [...n, { id: Date.now(), data: image64, name: fileName }])
            making = 0;
            file = "";
            fileName = "";
...
<div class="z-50 p-3 flex justify-between sticky top-0">
      ...
      {/if}
      {#if making == 1}
      <div transition:slide class="mr-2 bg-gradient-to-r from-cyan-500 to-cyan-500 py-1 px-2 rounded-lg text-white font-main">
        제작중
      </div>
  {/if}
  ...
  <div class="flex items-center w-[150px] bg-slate-100 rounded-lg row-end">
    ...
    <div style="flex-shrink: 0;">
        <button type="submit" on:click={onSubmit} class="flex items-center justify-center w-12 h-8 text-white rounded-r-lg {file ? 'bg-cyan-500 hover:bg-blue-900' : 'bg-gray-500 cursor-not-allowed'}">
            {#if making == 0}
  ...
  <div class="min-w-[20rem] px-3 pb-3 flex flex-col">
    ...
    <ImageBlock base64={component.data} name={component.name} on:deleteComponent={() => {
  ...
  <div class="no-select p-1 flex justify-between w-full fixed bottom-0 bg-gradient-to-r from-yellow-500 to-yellow-200 font-main text-black text-lg">
      ...
      <button class="flex text-black px-2" onclick="window.open('https://github.com/JungyoKim/AUTOCODER')">
